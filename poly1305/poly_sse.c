       ...
       size_t bytes)
{
	...
	if (bytes & 16) {
                _mm_store_si128((__m128i *) dst,
                                _mm_loadu_si128((__m128i *) (dst + offset)));
                dst += 16;
        }
                  ...
                  size_t bytes)
{
	...
	st->pad[0] = *(const uint64_t *) (key + 16);
	st->pad[1] = *(const uint64_t *) (key + 24);

	rt0 = r0;
	rt1 = r1;
	rt2 = r2;

        for (loops = 2, R = st->R2, limit = 16;
             loops && bytes > limit;
...
poly1305_update_sse(struct poly1305_state_s *st,
                    ...
                    size_t bytes)
{
	__attribute__((aligned(64))) __m128i HIBIT =
                _mm_shuffle_epi32(_mm_cvtsi32_si128(1 << 24),
                                  _MM_SHUFFLE(1, 0, 1, 0));
	const __m128i MMASK =
                _mm_shuffle_epi32(_mm_cvtsi32_si128((1 << 26) - 1),
                                  _MM_SHUFFLE(1, 0, 1, 0));
	...
	if (!(st->flags & poly1305_started)) {
		/* H = [Mx,My] */
		T5 = _mm_unpacklo_epi64(_mm_loadl_epi64((const __m128i *) (m + 0)),
                                        _mm_loadl_epi64((const __m128i *) (m + 16)));
		T6 = _mm_unpacklo_epi64(_mm_loadl_epi64((const __m128i *) (m + 8)),
                                        _mm_loadl_epi64((const __m128i *) (m + 24)));
		H0 = _mm_and_si128(MMASK, T5);
		H1 = _mm_and_si128(MMASK, _mm_srli_epi64(T5, 26));
		T5 = _mm_or_si128(_mm_srli_epi64(T5, 52),
                                  _mm_slli_epi64(T6, 12));
	...
	if (bytes >= 64) {
		...
		while (bytes >= 64) {
			...
			v42 = H2; v42 = _mm_mul_epu32(v42, T14);
			...
			v43 = H1; v43 = _mm_mul_epu32(v43, T15);
			...
			/* H += [Mx',My'] */
			T5 = _mm_loadu_si128((const __m128i *) (m + 32));
			T6 = _mm_loadu_si128((const __m128i *) (m + 48));
	...
	if (bytes >= 32) {
		...
		/* H += [Mx,My] */
		if (m) {
			T5 = _mm_loadu_si128((const __m128i *) (m + 0));
			T6 = _mm_loadu_si128((const __m128i *) (m + 16));
			T7 = _mm_unpacklo_epi32(T5, T6);
			T8 = _mm_unpackhi_epi32(T5, T6);
                 ...
                 size_t leftover)
{
	...
	if (st->flags & poly1305_started) {
		...
		poly1305_update_sse(st, NULL, 32);
	}
